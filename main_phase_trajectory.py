...
Runs the per-model phase-trajectory pipeline for multiple seeds:
  ...
  3) Hand plotting off to plot_all.sh
...
(SharedGate/DiagGate access-route test; models selected by launcher).
...
Directory layout produced:
  <outdir>/<optimizer>/
      seed_0042/{const,shared,diag}/...
      seed_0123/{const,shared,diag}/...
      seed_0321/{const,shared,diag}/...
      aggregated/{const,shared,diag}/phase_trajectory.csv   (mean ± se)
"""
...
# ============================================================
# Step 3: Plotting handoff
# ============================================================

def run_plotting_handoff(agg_dir: str, outdir: str, dpi: int, extra_plot_args: Dict):
    """Log the public plotting handoff.

    Paper-facing plots are generated by plot_all.sh after simulations finish.
    Keeping this as a no-op preserves old direct invocations that omit
    --skip_plot, while avoiding a second plotting stack with stale semantics.
    """
    del agg_dir, outdir, dpi, extra_plot_args
    log("Plotting is handled by plot_all.sh; skipping deprecated in-run plotting step.")
    return True
...
def parse_args():
    ...
    p.add_argument("--experiment_tag", type=str, default="phase_trajectory",
                   help="Prefix for orchestration config manifests, e.g. exp1 or exp2")
    ...
    # Model selection for the current experiment.
    p.add_argument("--models", type=str, default="shared,diag",
                   help="Comma-separated model names (all handled by unified run_phase_trajectory.py). "
                        "Default is the Exp2 access-route set (SharedGate/DiagGate); "
                        "the launchers pass the experiment-specific list explicitly.")
    ...
    # Heavy-tailed forcing injection. Manuscript Path A uses update mode.
    ...
    p.add_argument("--forcing_tail_k_frac", type=float, default=0.08,
                   help="Genuine-tail order-statistic fraction (k = k_frac * n) for the EVI estimators.")
    ...
    p.add_argument("--skip_plot", action="store_true",
                   help="Deprecated compatibility flag; paper plots are generated by plot_all.sh")
    p.add_argument("--plot_only", action="store_true",
                   help="Skip simulation; rerun deferred analysis and aggregation from saved seeds")
    p.add_argument("--tau_cap", type=float, default=1e6,
                   help="Deprecated plotting compatibility option; ignored by this runner")
    p.add_argument("--min_r2", type=float, default=None,
                   help="Deprecated plotting compatibility option; ignored by this runner")
    p.add_argument("--min_beta_r2", type=float, default=None,
                   help="Deprecated plotting compatibility option; ignored by this runner")
...
def build_common_args(args) -> Dict:
    ...
    d = {
        ...
        "inject_noise_clip": args.inject_noise_clip,
        ...
        "slow_mode_q_low": args.slow_mode_q_low,
        "forcing_tail_max_samples": args.forcing_tail_max_samples,
        "forcing_tail_bootstrap_B": args.forcing_tail_bootstrap_B,
        "forcing_tail_ci_B": args.forcing_tail_ci_B,
        "forcing_tail_k_min": args.forcing_tail_k_min,
        "forcing_tail_k_frac": args.forcing_tail_k_frac,
        "forcing_tail_k_max_frac": args.forcing_tail_k_max_frac,
        "forcing_tail_substantive_alpha": args.forcing_tail_substantive_alpha,
        "forcing_tail_gaussian_test_alpha": args.forcing_tail_gaussian_test_alpha,
...
def _load_run_config_for_plot_only(args):
    ...
    cfg_path = os.path.join(opt_dir, f"{args.experiment_tag}_config.json")
    if not args.plot_only:
        return args
    if not os.path.isfile(cfg_path):
        raise FileNotFoundError(
            "plot_only requires the original simulation manifest so deferred "
            "analysis uses the same task, sequence length, lag grid, and "
            f"checkpoint schedule. Expected: {cfg_path}"
        )
...
def _model_seed_dirs_with_final_artifacts(seed_dirs: List[str], model_name: str) -> List[str]:
    """Return seed directories whose model has complete final plot-only artifacts."""
    complete = []
    for seed_dir in seed_dirs:
        model_dir = os.path.join(seed_dir, model_name)
        required = [
            os.path.join(model_dir, f"{model_name}_final_phase.json"),
            os.path.join(model_dir, f"{model_name}_envelope.csv"),
            os.path.join(model_dir, f"{model_name}_envelope_fit.json"),
            os.path.join(model_dir, f"{model_name}_envelope_fit_curves.csv"),
        ]
        if all(os.path.isfile(path) for path in required):
            complete.append(seed_dir)
    return complete


def _report_plot_only_final_artifacts(seed_dirs: List[str], models: List[str]) -> Dict[str, List[str]]:
    """Report complete seed/model pairs after deferred plot-only analysis.

    A resumed run can leave a single seed/model incomplete while the aggregate
    scripts still find enough checkpoint rows to produce misleading figures.
    We allow per-model seed counts to differ, but every aggregate below uses
    only seeds with complete final envelope/phase artifacts for that model.
    """
    complete_by_model: Dict[str, List[str]] = {}
    for seed_dir in seed_dirs:
        seed_tag = os.path.basename(seed_dir.rstrip(os.sep))
        for model_name in models:
            ...
            if missing:
                log(
                    f"  [plot_only] excluding incomplete {seed_tag}/{model_name}: "
                    f"{len(missing)} final artifact(s) missing"
                )
            else:
                complete_by_model.setdefault(model_name, []).append(seed_dir)

    for model_name in models:
        complete_by_model.setdefault(model_name, [])
        n_complete = len(complete_by_model[model_name])
        n_requested = len(seed_dirs)
        log(
            f"  [plot_only] {model_name}: using {n_complete}/{n_requested} "
            "seed(s) with complete final artifacts"
        )
        if n_complete == 0:
            raise RuntimeError(
                f"No complete final plot-only artifacts found for model={model_name}."
            )
    return complete_by_model
...
def main():
    ...
    config_name = (
        f"{args.experiment_tag}_analysis_config.json"
        if args.plot_only else
        f"{args.experiment_tag}_config.json"
    )
    ...
    if args.plot_only:
        ...
        complete_seed_dirs_by_model = _report_plot_only_final_artifacts(seed_dirs, all_models)
    else:
        complete_seed_dirs_by_model = {
            model_name: _model_seed_dirs_with_final_artifacts(seed_dirs, model_name)
    ...
    for model_name in all_models:
        model_seed_dirs = complete_seed_dirs_by_model.get(model_name) or seed_dirs
        agg = aggregate_trajectories(model_seed_dirs, model_name)
        if not agg:
    ...
    for model_name in all_models:
        model_seed_dirs = complete_seed_dirs_by_model.get(model_name) or seed_dirs
        lc_agg = aggregate_learning_curves(model_seed_dirs, model_name)
        if not lc_agg:
    ...
    if seed_dirs:
        ...
        for model_name in all_models:
            model_seed_dirs = complete_seed_dirs_by_model.get(model_name) or seed_dirs
            last_seed_dir = model_seed_dirs[-1] if model_seed_dirs else None
            if last_seed_dir is None:
                continue
            ...
            aggregate_final_artifacts(model_seed_dirs, model_name, mdir_agg)
    ...
    summary_cols = ["model", "n_seeds",
                    ...
                    "forcing_alpha_hat_mean", "forcing_alpha_hat_se",
                    "forcing_gaussian_p_value_mean", "forcing_gaussian_p_value_se",
                    "forcing_gaussian_p_value_floor_mean", "forcing_gaussian_p_value_floor_se",
                    "forcing_gaussian_p_value_at_floor_mean", "forcing_gaussian_p_value_at_floor_se",
                    "forcing_gaussian_reject_mean", "forcing_gaussian_reject_se",
                    "forcing_alpha_detectably_heavy_mean", "forcing_alpha_detectably_heavy_se",
                    "forcing_alpha_substantively_heavy_mean", "forcing_alpha_substantively_heavy_se",
                    "forcing_resolvably_heavy_mean", "forcing_resolvably_heavy_se",
    ...
    for model_name in all_models:
        ...
        row = [model_name, n_s]
        for col in [
            "beta_hat", "beta_median", "p_beta_lt1", "beta_env",
            "forcing_alpha_hat",
            "forcing_gaussian_p_value",
            "forcing_gaussian_p_value_floor",
            "forcing_gaussian_p_value_at_floor",
            "forcing_gaussian_reject",
            "forcing_alpha_detectably_heavy",
            "forcing_alpha_substantively_heavy",
            "forcing_alpha_resolvably_heavy",
            "alpha_hat", "tau_mean", "delta_zeta",
        ]:
            m_key = f"{col}_mean"
            se_key = f"{col}_se"
    ...
    log(f"  {'Model':<12} {'n':>4}  {'beta_hat':>8}  {'b_med':>8}  {'P(b<1)':>8}  {'beta_env':>8}  {'a_slow':>8}  {'pG':>6}  {'H_sub':>6}  {'tau_mean':>10}  {'Dzeta':>8}")
    log(f"  {'-'*12} {'-'*4}  {'-'*8}  {'-'*8}  {'-'*8}  {'-'*8}  {'-'*8}  {'-'*6}  {'-'*6}  {'-'*10}  {'-'*8}")
    for r in summary_rows:
        ...
        be_m, be_se = r[8], r[9]
        fa_m, fa_se = r[10], r[11]
        pg_m, pg_se = r[12], r[13]
        fh_m, fh_se = r[22], r[23]
        tm_m, tm_se = r[28], r[29]
        dz_m, dz_se = r[30], r[31]
        log(f"  {model_name:<12} {n_s:>4}  {bh_m:>5.2f}+/-{bh_se:.2f}  {bmed_m:>5.2f}+/-{bmed_se:.2f}  {pbl_m:>5.2f}+/-{pbl_se:.2f}  {be_m:>5.2f}+/-{be_se:.2f}  {fa_m:>5.2f}+/-{fa_se:.2f}  {pg_m:>4.2f}+/-{pg_se:.2f}  {fh_m:>4.2f}+/-{fh_se:.2f}  {tm_m:>7.1f}+/-{tm_se:.1f}  {dz_m:>5.2f}+/-{dz_se:.2f}")
    ...
    if not args.skip_plot:
        ...
        log(f"Running plotting handoff on {agg_dir} -> {plots_dir}")
        run_plotting_handoff(agg_dir, plots_dir, args.dpi, extra_plot_args)
